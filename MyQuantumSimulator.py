...
class Circuit:
    ...
    def apply_multiple_gate(self,control_list,target_list,gate):
        ...
        for i in range(2**len(control_list)):
            ...
            for j in range(self.n):
                ...
                if j in control_list:
                    gate_matrix = np.kron(gate_matrix,controls[int(binary_str[k])])
                    
                    k+=1
                elif(i < (2**len(control_list)-1) or j not in target_list ):
                    gate_matrix = np.kron(gate_matrix,self.I)
        ...
        self.state_vector = np.dot(linear_gate_matrix, self.state_vector)
